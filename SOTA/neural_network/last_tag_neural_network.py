...
class Neural_network_trainer():
    ...
    def train(self, learning_rate=0.01, batch_size=512, max_epochs=100,
        ...
        w2 = theano.shared(value=np.random.uniform(-lim,lim,((n_window)*n_emb+tag_dim,self.n_out)).
        ...
        tag_lim = np.sqrt(6./(n_window+tag_dim))
        ...
        def forward_pass(weight_x, prev_pred, bias_1, weight_2, bias_2, weight_t):
        ...
        if use_scan:
            ...
            [y_pred,out], _ = theano.scan(fn=forward_pass,
                                    sequences=w_x,
