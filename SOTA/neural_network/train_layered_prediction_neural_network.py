...
def filter_tags_to_predict(x_train, y_train, x_test, y_test, label2index, index2label, tags):
    ...
    # for tag in tags_not_included:
    #     value = label2index[tag]
    #     del label2index[tag]
    #     del index2label[value]
...
def use_testing_dataset(nn_class,
    ...
    if tags:
        tags = get_param(tags)
        y_train, y_test, label2index, index2label = filter_tags_to_predict(x_train, y_train, x_test, y_test, label2index, index2label, tags)
