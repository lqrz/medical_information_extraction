...
class Hidden_Layer_Hierarchical_net(A_neural_network):
    ...
    def __init__(self,
        ...
        super(Hidden_Layer_Hierarchical_net, self).__init__(**kwargs)
    ...
    def train_with_sgd(self, learning_rate=0.01, max_epochs=100,
        ...
        if w1_values is not None:
            w1 = theano.shared(value=np.array(w1_values).astype(dtype=theano.config.floatX), name='w1', borrow=True)
            b1 = theano.shared(value=np.zeros((w1_values.shape[1])).astype(dtype=theano.config.floatX), name='b1',
                               borrow=True)
            w2 = theano.shared(value=utils.NeuralNetwork.initialize_weights(n_in=w1_values.shape[1],
                                                                            n_out=self.n_out, function='tanh').
                               astype(dtype=theano.config.floatX), name='w2', borrow=True)
        elif n_hidden is not None:
            w1 = theano.shared(value=utils.NeuralNetwork.initialize_weights(n_in=self.n_emb, n_out=n_hidden, function='tanh').astype(dtype=theano.config.floatX), name='w1', borrow=True)
            b1 = theano.shared(value=np.zeros((n_hidden)).astype(dtype=theano.config.floatX), name='b1',
                               borrow=True)
            ...
            w2 = theano.shared(value=utils.NeuralNetwork.initialize_weights(n_in=self.n_window * self.n_emb,
                                                                            n_out=self.n_out, function='tanh').
                               astype(dtype=theano.config.floatX), name='w2', borrow=True)
