...
class Vector_tag_CW_MLP_neural_network_trainer_random(object):
    ...
    def train(self, learning_rate=0.01, batch_size=128, max_epochs=100,
        ...
        w1 = theano.shared(value=np.array(self.pretrained_embeddings).astype(dtype=theano.config.floatX),
                           name='w1', borrow=True)
        w2 = theano.shared(value=utils.NeuralNetwork.initialize_weights(n_in=n_window*(n_emb+self.tag_dim), n_out=self.n_out, function='tanh').
                           astype(dtype=theano.config.floatX),
                           name='w2', borrow=True)
        b1 = theano.shared(value=np.zeros((n_window*(n_emb+self.tag_dim))).astype(dtype=theano.config.floatX), name='b1', borrow=True)
        ...
        # test_scan = theano.function(inputs=[idxs, initial_tags], outputs=out, updates=[])
